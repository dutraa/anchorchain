...
const simFundingFactoidPrivateKeyHex = "FB3B471B1DCDADFEB856BD0B02D8BF49ACE0EDD372A3D9F2A95B78EC12A324D6"
...
func FundWalletTOFF(st *state.State, timeOffsetInMilliseconds int64, amt uint64) (error, string) {
	inSec, _ := primitives.HexToHash(simFundingFactoidPrivateKeyHex)                                 // private key or FCT Source
...
// FundECWalletFromBank buys entry credits for the provided EC address using the
// built-in sim wallet funding source that already exists for dev/test flows.
func FundECWalletFromBank(st *state.State, userECOut string, credits uint64) (error, string) {
	inSec, _ := primitives.HexToHash(simFundingFactoidPrivateKeyHex)
	outEC := factoid.NewAddress(primitives.ConvertUserStrToAddress(userECOut))
	amt := credits * st.GetFactoshisPerEC()
	return FundECWallet(st, inSec, outEC, amt)
}
